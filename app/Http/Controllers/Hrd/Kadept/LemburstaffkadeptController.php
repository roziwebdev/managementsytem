...
class LemburstaffkadeptController extends Controller
{
    ...
    $lemburstaffs = $query->orderByDesc('id')->paginate(10);

        return view('role.hrd.lemburstaff.lemburstaffkadept.listlembur',compact('lemburstaffs','karyawanstaff'));
    }
    public function store(Request $request){
        
        // foreach ($request->namakaryawan as $namakaryawan) {
        //     $existingLembur = Lemburstaff::where('namakaryawan', 'like', '%' . $namakaryawan . '%')
