            ...
            <div class="col-12 stretch-card grid-margin" id="myForm">
                <div class="border rounded card">
                    <div class="border rounded shadow card-body">
                        ...
                        <form class="form-sample" method="POST" action="{{ route('splstaff.update', $lemburstaff->id) }}"
                            id="pricelistForm">
                            @method("PUT")
                            @csrf
                            @php
                            $namas = json_decode($lemburstaff->namakaryawan);
                            $jabatans = json_decode($lemburstaff->jabatan);
                            $mulais = json_decode($lemburstaff->mulai);
                            $istirahats = json_decode($lemburstaff->istirahat);
                            $berakhirs = json_decode($lemburstaff->berakhir);
                            $hasillemburs = json_decode($lemburstaff->hasillembur);
                            $departements = json_decode($lemburstaff->departement);
                            $totaljams = json_decode($lemburstaff->totaljam);
                            $keteranganpekerjaans = json_decode($lemburstaff->keteranganpekerjaan);
                            $units = json_decode($lemburstaff->unit);
                            $pekerjaans = json_decode($lemburstaff->pekerjaan);
                            $combinedData =
                                    ...
                                    <div class="mt-3 col-md ">
                                        <div class="mb-3" style="font-size: 14px">
                                            ...
                                            <input type="text" class="form-control form-control-sm clickable" name="namakaryawan[]" required
                                                id="input1" list="kar" value="{{ $data[0] }}">
                                        </div>
                                    </div>
                                    <div class="mt-3 col-md ">
                                        <div class="mb-3" style="font-size: 14px">
                                            <label for="" class="form-label fw-bold">Jabatan</label>
                                            <input type="text" class="form-control form-control-sm clickable" name="jabatan[]" required id="input1"
                                                list="kar" value="{{ $data[1] }}">
                                        </div>
                                    </div>
                                    <div class="mt-3 col-md ">
                                        <div class="mb-3" style="font-size: 14px">
                                            ...
                                            <input type="text" class="form-control form-control-sm clickable" name="departement[]" required
                                                id="input1" list="kar" value="{{ $data[2] }}">
                                        </div>
                                    </div>
